...
# DAI
const DAI = 0x04831207050e4b35a50dbb51a389b336c97720f4fd1fdf489b92dab9409baf99

#Contract addresses
const XTokenContractAddress = 0x04b3f2476fbd0e7eeccb18576b3768501a856030ee7468b5302a4c342ad097e1
...
func balance_of{
    ...
    let (share_of_user) = user_share_balance.read(user_address, DAI)
    let (total_asset_amount) = get_total_asset_amount()
    let (total_shares_amount) = total_shares.read(DAI)
...
func get_asset_balance{
    ...
    let (this_contract_address) = get_contract_address()
    let (balance : Uint256) = IERC20.balanceOf(contract_address=DAI, account=this_contract_address)
    return (balance)
end
...
func approve_protocol_tokens{
    ...
    IERC20.approve(contract_address=DAI,spender=XTokenContractAddress,amount=infinite)
...
func deposit{
    ...
    let (this_contract_address) = get_contract_address()
    IERC20.transferFrom(contract_address = DAI, sender = caller_address, recipient = this_contract_address, amount = amount)
    
    # Increase User Shares
    let (share_of_deposit_user) = user_share_balance.read(caller_address, DAI)
    let (new_depositor_shares,_) = uint256_add(share_of_deposit_user, amount)
    user_share_balance.write(caller_address, DAI, new_depositor_shares)
    
    #Increase Total Shares
    let (local_total_shares) = total_shares.read(DAI)
    let (new_total_shares,_) = uint256_add(local_total_shares,amount)
    total_shares.write(DAI,new_total_shares)
...
func withdraw_amount{
    ...
    let (total_shares_amount) = total_shares.read(DAI)
    let (total_asset_amount) = get_total_asset_amount()
    let (user_share) = user_share_balance.read(caller_address,DAI)
    let (balance_of_user) = balance_of(user_address = caller_address)
    ...
    if enough_cash_to_send != 0 :
        ...
        user_share_balance.write(caller_address, DAI, new_user_share)

        IERC20.transfer(contract_address=DAI, recipient=caller_address, amount=amount)

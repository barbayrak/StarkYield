...
#Contract addresses
const XTokenContractAddress = 0x054f0b25c4b916cf31788da91478e5298f3c016d7473e6cbdb1e7f540253085f
...
#1 ETH
const LEVERAGE_MIN_BALANCE = 100000000000000
...
func balance_of{
    ...
    let (share_of_user) = user_share_balance.read(user_address,ETH)
    let (total_asset_amount) = get_total_asset_amount()
    let (total_shares_amount) = total_shares.read(ETH)
...
func get_asset_balance{
    ...
    let (this_contract_address) = get_contract_address()
    let (balance : Uint256) = IERC20.balanceOf(contract_address=ETH, account=this_contract_address)
    return (balance)
end
...
func approve_protocol_tokens{
    ...
    IERC20.approve(contract_address=ETH,spender=XTokenContractAddress,amount=infinite)
...
func deposit{
    ...
    let (this_contract_address) = get_contract_address()
    IERC20.transferFrom(contract_address = ETH, sender = caller_address, recipient = this_contract_address, amount = amount)
    
    # Increase User Shares
    let (share_of_deposit_user) = user_share_balance.read(caller_address, ETH)
    let (new_depositor_shares,_) = uint256_add(share_of_deposit_user, amount)
    user_share_balance.write(caller_address, ETH, new_depositor_shares)
    
    #Increase Total Shares
    let (local_total_shares) = total_shares.read(ETH)
    let (new_total_shares,_) = uint256_add(local_total_shares,amount)
    total_shares.write(ETH,new_total_shares)
...
func withdraw_amount{
    ...
    let (total_shares_amount) = total_shares.read(ETH)
    let (total_asset_amount) = get_total_asset_amount()
    let (user_share) = user_share_balance.read(caller_address,ETH)
    let (balance_of_user) = balance_of(user_address = caller_address)
    ...
    if enough_cash_to_send != 0 :
        ...
        user_share_balance.write(caller_address, ETH, new_user_share)

        IERC20.transfer(contract_address=ETH, recipient=caller_address, amount=amount)
